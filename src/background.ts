...
async function fetchAllPRs(): Promise<PullRequest[]> {
    console.log('Background - Fetching all PRs');
    ...
    await chrome.storage.local.set({
        'pr-viewer-prs': prs,
        'pr-viewer-last-fetch': new Date().toISOString()
    });
    return prs;
}

chrome.runtime.onInstalled.addListener(() => {
    console.log('PR Viewer installed');
...
function startRefreshInterval(minutes: number) {
    console.log("executing startRefreshInterval");
    if (refreshInterval) {
